# Do Worktree Examples

Purpose: Real-world usage examples and patterns for do-worktree skill integration with Do workflow.
...
/do:1-plan "User Authentication System" --worktree

# Output:
# SPEC created: SPEC-AUTH-001
# Worktree created: ~/workflows/Do/SPEC-AUTH-001
#
# Next steps:
# 1. Switch to worktree: do-worktree switch SPEC-AUTH-001
# 2. Or use shell eval: eval $(do-worktree go SPEC-AUTH-001)
# 3. Start development: /do:2-run SPEC-AUTH-001

# Development Phase - Implement in isolated environment
eval $(do-worktree go SPEC-AUTH-001)
/do:2-run SPEC-AUTH-001

# Sync Phase - Synchronize and integrate
do-worktree sync SPEC-AUTH-001
/do:3-sync SPEC-AUTH-001

# Cleanup Phase - Remove completed worktree
do-worktree remove SPEC-AUTH-001
```
...
do-worktree new SPEC-AUTH-001 "User Authentication"
do-worktree new SPEC-PAY-001 "Payment Processing"
do-worktree new SPEC-DASH-001 "Dashboard Analytics"
...
```bash
# /do:2-run automatically detects worktree environment
/do:2-run SPEC-AUTH-001
...
do-worktree status SPEC-AUTH-001
do-worktree sync SPEC-AUTH-001 --include "src/"
...
Skill("do-worktree") # Load worktree patterns
...
if project_config.get("worktree_enabled"):
 from do_worktree import WorktreeManager
...
/do:1-plan "$SPEC_DESCRIPTION" --worktree --spec-id "$SPEC_ID"

# Check if worktree was created successfully
if do-worktree list --format json | jq -r ".worktrees[\"$SPEC_ID\"]" > /dev/null; then
...
do-worktree config set worktree_root "$WORKTREE_ROOT"
do-worktree config set registry_type team

echo " Team worktree configuration completed"
echo "Team members can now join with: do-worktree join --team $TEAM_NAME"
```
...
DEBUG=true""",
 ...
 # Save template
 template_path = Path.home() / ".do-worktree/templates" / "fullstack.json"
...
# do-worktree new SPEC-FULL-001 "Fullstack Application" --template fullstack
```
...
class WorktreeAutomation:
 ...
 result = subprocess.run([
 "do-worktree", "list", "--status", "active", "--format", "json"
 ], capture_output=True, text=True, check=True)
 ...
 'action': 'do-worktree clean --stale --days 30'
 })
 ...
 'action': 'do-worktree optimize --analyze'
 })
...
do-worktree new SPEC-DEBUG-001 "Debug Test" --verbose

# Problem 2: Worktree sync conflicts
echo " Resolving sync conflicts..."

# Check sync status
do-worktree status SPEC-CONFLICT-001

# Interactive conflict resolution
do-worktree sync SPEC-CONFLICT-001 --interactive

# Force sync (if appropriate)
do-worktree sync SPEC-CONFLICT-001 --force

# Problem 3: Worktree registry corruption
echo " Repairing worktree registry..."

# Backup current registry
cp ~/.workflows/PROJECT/.do-worktree-registry.json ~/.workflows/PROJECT/.do-worktree-registry.json.backup

# Rebuild registry from worktree directories
do-worktree config set registry_rebuild true
do-worktree list --rebuild-registry
...
Version: 1.0.0
Last Updated: 2025-11-29
Examples: Real-world usage patterns for do-worktree integration
