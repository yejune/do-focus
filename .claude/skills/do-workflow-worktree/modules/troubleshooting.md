...
- Integration Issues: Do workflow integration and command coordination

Quick Diagnostics:
- Check worktree status: do-worktree status --all
- Verify registry integrity: do-worktree registry validate
...
Prevention:
- Always use do-worktree remove instead of manual directory deletion
...
Symptoms:
- do-worktree commands fail with JSON parsing errors
...
- Validate the rebuilt registry with do-worktree status --all

### Orphaned Registry Entries

Symptoms:
- Registry lists worktrees that no longer exist on disk
- do-worktree status shows worktrees as missing
...
Resolution Steps:
1. Run do-worktree status --all to identify orphaned entries
2. For each orphaned entry, confirm the directory truly does not exist
3. Remove orphaned entries from registry using do-worktree registry prune
...
- Use do-worktree commands for standard operations
...
### Do Command Coordination Failures

Symptoms:
- /do:1-plan does not create expected worktree
- /do:2-run cannot find worktree for SPEC
- Workflow commands operate on wrong worktree

Root Causes:
- SPEC ID not matching worktree ID format
- Worktree was created manually without Do integration
- Configuration mismatch between Do and worktree settings
- Working directory context not in expected location

Resolution Steps:
1. Verify SPEC ID format matches expected worktree naming convention
2. Ensure worktree was created through Do workflow or properly registered
3. Check do configuration for worktree integration settings
...
Status Commands:
- do-worktree status --all: Shows all worktrees with sync status
- git worktree list: Native Git worktree listing
- do-worktree status SPEC-ID --detailed: Detailed status for specific worktree

Registry Commands:
- do-worktree registry validate: Checks registry integrity
- do-worktree registry prune: Removes orphaned entries
- do-worktree registry export: Exports registry for backup
...
Cleanup Commands:
- do-worktree clean --dry-run: Preview cleanup without changes
- do-worktree clean --merged-only: Clean only merged worktrees
- git worktree prune: Remove stale Git worktree metadata

Recovery Commands:
- do-worktree registry rebuild: Reconstruct registry from directories
- do-worktree remove SPEC-ID --keep-branch: Remove worktree, preserve branch
...
Weekly Tasks:
- Run do-worktree status --all to check for issues
...
Creation:
- Always use do-worktree new instead of manual Git commands
...
Removal:
- Use do-worktree remove instead of manual deletion
