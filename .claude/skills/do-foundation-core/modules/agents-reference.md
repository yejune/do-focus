# Agents Reference - Do Agent Catalog

Purpose: Complete reference catalog of Do's 26 specialized agents with `{domain}-{role}` naming convention and 7-tier hierarchy.
...
Do delegates ALL tasks to specialized agents. 26 agents organized in 7 tiers:
...
All Do agents follow consistent naming:
...
Core command processors directly bound to Do commands.

| Agent | Command | Purpose |
|-------|---------|---------|
| `workflow-project` | `/do:0-project` | Project initialization and setup |
| `workflow-spec` | `/do:1-plan` | EARS SPEC generation and planning |
| `workflow-tdd` | `/do:2-run` | TDD RED-GREEN-REFACTOR execution |
| `workflow-docs` | `/do:3-sync` | Documentation generation and synchronization |
...
|-------|---------|---------|
| `core-planner` | `/do:2-run` Phase 1 | SPEC analysis and execution strategy |
...
Meta-generation agents for Do development.
...
Use Case: When developing Do itself (not for end-user projects)
...
Note: These are Claude Code built-in agents, not Do custom agents.
...
1. Agent-First: Do NEVER executes tasks directly. ALWAYS delegates via Task()
...
|----------|--------|---------|
| Language (Separated) | do-lang-python, do-lang-typescript, do-lang-systems, do-lang-jvm, do-lang-mobile | Domain-specific language skills for 40-60% token savings |
| Platform (Separated) | do-platform-auth, do-platform-database, do-platform-deploy | Domain-specific platform skills for 30-50% token savings |
| Foundation | do-foundation-core, do-foundation-claude, do-foundation-context, do-foundation-quality | Core principles and quality gates |
| Workflow | do-workflow-spec, do-workflow-project, do-workflow-testing, do-workflow-jit-docs | Workflow automation and testing |
| Domain | do-domain-backend, do-domain-frontend, do-domain-database, do-domain-uiux | Domain expertise patterns |

Language Skills Selection Guide:

| Language Skill | Coverage | Use When |
|----------------|----------|----------|
| do-lang-python | Python 3.13, FastAPI, Django, pytest | Backend APIs, data science, automation |
| do-lang-typescript | TypeScript 5.9, React 19, Next.js 16, tRPC | Frontend, full-stack web development |
| do-lang-systems | Go 1.23, Rust 1.91, Fiber, Axum | Microservices, CLI tools, systems programming |
| do-lang-jvm | Java 21, Kotlin 2.0, Scala 3.4, Spring | Enterprise applications, big data |
| do-lang-mobile | Swift 6, Kotlin Android, Flutter 3.24 | iOS, Android, cross-platform mobile |

Platform Skills Selection Guide:

| Platform Skill | Providers | Use When |
|----------------|-----------|----------|
| do-platform-auth | Auth0, Clerk, Firebase Auth | Authentication implementation |
| do-platform-database | Supabase, Neon, Convex, Firestore | Database platform integration |
| do-platform-deploy | Vercel, Railway | Deployment and CI/CD |

Note: Agents now use specific skills based on their domain. Cross-language agents include do-lang-python and do-lang-typescript by default.
...
Skills:
- [do-foundation-core](../SKILL.md) - Parent skill (this module is part of it)
- [do-foundation-context](../../do-foundation-context/SKILL.md) - Token budget and session state
- [do-foundation-claude](../../do-foundation-claude/SKILL.md) - Claude Code configuration
...
Commands:
- `/do:0-project` → `workflow-project`
- `/do:1-plan` → `workflow-spec`
- `/do:2-run` → `workflow-tdd`
- `/do:3-sync` → `workflow-docs`
