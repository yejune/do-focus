# Do Foundation Core - Modules Directory

Purpose: Extended documentation modules for do-foundation-core skill, providing deep dives into each foundational principle.
...
This directory contains 9 comprehensive modules covering Do's foundational principles and execution rules:
...
Quick Access:
- SPEC generation (/do:1-plan)
- EARS format patterns
- TDD implementation (/do:2-run)
- Documentation sync (/do:3-sync)
...
Agent Catalog - Complete reference of Do's 26 specialized agents with 7-tier hierarchy.
...
Command Catalog - Complete reference for Do's 6 core commands in SPEC-First TDD workflow.

Quick Access:
- /do:0-project (Project init)
- /do:1-plan (SPEC generation)
- /do:2-run (TDD implementation)
- /do:3-sync (Documentation)
- /do:9-feedback (Improvement)
- /do:99-release (Deployment)
...
skill_path = Path(".claude/skills/do-foundation-core")
...
class ModuleSearch:
 ...
 modules_dir = Path(".claude/skills/do-foundation-core/modules")
 results = []
...
Skills:
- do-foundation-core (parent skill)
- do-cc-skill-factory (skill creation)
