...
Purpose: Security policies, execution constraints, and Git workflow strategies governing Do agent behavior.
...
Do operates under strict execution rules:
...
| `Skill()` | Knowledge invocation | `Skill("do-foundation-core")` |
...
Do automatically adjusts Git workflow based on `config.json` settings.
...
Do's Behavior (prompt_always=true):
1. When running `/do:1-plan`, user prompted: "Create branch?"
...
Do's Behavior (prompt_always=false):
...
2. Running `/do:2-run`: TDD commits + auto push
3. Running `/do:3-sync`: Doc commits + suggest PR creation (user choice)
...
Do's Behavior (prompt_always=false, auto_enabled=false):
1. When running `/do:1-plan`, user prompted once: "Enable automatic branch creation?"
...
Do's Behavior (prompt_always=false, auto_enabled=true):
...
2. Running `/do:2-run`: TDD commits + auto push (to feature branch)
3. Running `/do:3-sync`: Doc commits + prepare PR
...
Do's Behavior (prompt_always=false, auto_enabled=false):
1. When running `/do:1-plan`, user prompted once: "Enable automatic branch creation and Draft PR creation?"
...
Execute `/clear` after `/do:1-plan` completion in ALL modes.
...
1. Immediately after `/do:1-plan` (mandatory)
...
- Create documentation in `.do/` directory
...
This comprehensive set of execution rules ensures that Do operates securely, efficiently, and in compliance with industry standards while maintaining high quality output.
