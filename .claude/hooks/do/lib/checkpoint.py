...
def detect_risky_operation(tool_name: str, tool_args: dict[str, Any], cwd: str) -> tuple[bool, str]:
    ...
    if tool_name in ("Edit", "Write"):
        ...
        critical_files = [
            "CLAUDE.md",
            "config.json",
            ".claude/skills/do-core-dev-guide/reference.md",
            ".claude/skills/do-core-spec-metadata-extended/reference.md",
...
def create_checkpoint(cwd: str, operation_type: str) -> str:
    ...
    Notes:
        ...
        - Automatically record checkpoint logs (.do/checkpoints.log)
...
def log_checkpoint(cwd: str, branch_name: str, operation_type: str) -> None:
    """Checkpoint log records (.do/checkpoints.log)
    ...
    Examples:
        ...
        # Add 1 line to .do/checkpoints.log
    ...
    """
    log_file = Path(cwd) / ".do" / "checkpoints.log"
...
def list_checkpoints(cwd: str, max_count: int = 10) -> list[dict[str, str]]:
    """Checkpoint list (parsing .do/checkpoints.log)

    Returns a list of recently created checkpoints.
    Used in the SessionStart, /do:0-project restore command.
