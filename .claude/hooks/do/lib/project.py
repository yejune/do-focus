...
CACHE_DIR_NAME = ".do/cache"


def find_project_root(start_path: str | Path = ".") -> Path:
    """Find Do project root by searching upward for .do/config/config.yaml

    Traverses up the directory tree until it finds .do/config/config.yaml or CLAUDE.md,
    ...
    Notes:
        - Searches for .do/config/config.yaml first (most reliable)
    ...
    TDD History:
        ...
        - GREEN: Minimal upward search with .do/config/config.yaml detection
    ...
    for _ in range(max_depth):
        # Check for .do/config/config.yaml (primary indicator)
        if (current / ".do" / "config" / "config.yaml").exists():
            return current
...
def count_specs(cwd: str) -> dict[str, int]:
    ...
    Browse the .do/specs/ directory to find the number of SPEC Files and
    ...
    Returns:
        ...
        All 0 if .do/specs/ directory does not exist
...
def get_project_language(cwd: str) -> str:
    ...
    Notes:
        - Reads ``.do/config/config.yaml`` first for a quick answer.
        - Falls back to ``detect_language`` if configuration is missing.
        - Automatically finds project root to locate .do/config/config.yaml
    """
    # Find project root to ensure we read config from correct location
    project_root = find_project_root(cwd)
    config_path = project_root / ".do" / "config" / "config.yaml"
...
def _validate_project_structure(cwd: str) -> bool:
    """Validate that project has required Do structure

    Args:
        cwd: Project root directory path

    Returns:
        bool: True if .do/config/config.yaml exists, False otherwise
    """
    project_root = find_project_root(cwd)
    return (project_root / ".do" / "config" / "config.yaml").exists()


def get_version_check_config(cwd: str) -> dict[str, Any]:
    """Read version check configuration from .do/config/config.yaml
    ...
    try:
        ...
        # Extract do.version_check section
        do_config = config.get("do", {})
        version_check_config = do_config.get("version_check", {})

        # Read enabled flag (default: True)
        enabled = version_check_config.get("enabled", defaults["enabled"])

        # Read frequency (default: "daily")
        frequency = do_config.get("update_check_frequency", defaults["frequency"])
...
def get_package_version_info(cwd: str = ".") -> dict[str, Any]:
    """Check Do current and latest version with caching and offline support.
    ...
    # This prevents creating .do/cache in wrong locations when hooks run
    ...
    try:
        current_version = version("do-adk")
    ...
    try:
        with timeout_handler(1):
            url = "https://pypi.org/pypi/do-adk/json"
            ...
            with urllib.request.urlopen(req, timeout=0.8) as response:
                ...
                try:
                    ...
                    if not release_url:
                        ...
                        release_url = f"https://github.com/modu-ai/do-adk/releases/tag/v{result['latest']}"
    ...
    if result["current"] != "unknown" and result["latest"] != "unknown":
        try:
            ...
            if latest_parts > current_parts:
                ...
                result["upgrade_command"] = "uv tool upgrade do-adk"
