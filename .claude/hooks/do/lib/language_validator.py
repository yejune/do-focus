#!/usr/bin/env python3
"""Language Configuration Validator for Do
...
def load_config() -> Dict[str, Any]:
    """Load configuration from .do/config/config.json

    Returns:
        Configuration dictionary or empty dict if not found
    """
    config_path = Path(".do/config/config.json")
...
def validate_output_style_compatibility() -> Dict[str, Any]:
    ...
    r2d2_path = Path(".claude/output-styles/do/r2d2.md")
    ...
    if r2d2_path.exists():
        try:
            ...
            result["config_reading_present"] = ".do/config/config.json" in content
...
def validate_session_start_hook() -> Dict[str, Any]:
    ...
    hook_path = Path(".claude/hooks/do/session_start__show_project_info.py")
