...
DEFAULT_CONFIG = {
    "hooks": {
        ...
        "messages": {
            ...
            "config": {
                "missing": "❌ Project configuration not found - run /do:0-project",
        ...
        "version_check": {
            "pypi_url": "https://pypi.org/pypi/do-adk/json",
            "timeout_seconds": 1,
...
class ConfigManager:
    ...
    def __init__(self, config_path: Optional[Path] = None):
        ...
        Args:
            config_path: Path to configuration file (defaults to .do/config/config.yaml or config.json)
        """
        if config_path:
            ...
            yaml_path = project_root / ".do" / "config" / "config.yaml"
            json_path = project_root / ".do" / "config" / "config.json"
